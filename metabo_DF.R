...
if(!is.null(pathways_list)){
  ...
  if(length(setdiff(pathways_list,names(metaginfo$pathigraphs)))>0){
    #stop("ERROR: detected strange pathway id")
    print("ERROR: detected strange pathway id")
    pathways_list <- intersect(pathways_list,names(metaginfo$pathigraphs))
  }
  fpathigraphs <- metaginfo$pathigraphs[pathways_list]
  pathigraph.genes <- all.needed.genes(fpathigraphs)
} else {
  fpathigraphs <- metaginfo$pathigraphs
  pathigraph.genes <- metaginfo$all.genes
  pathways_list <- names(fpathigraphs)
}
...
if(dataset=="gex"){
  ...
  load(paste0(codebase,"/files/xref/",species,"/xref.rdata"))
  exp <- translate.matrix(exp,xref,verbose=T)
  exp <- normalize.data(exp,by.quantiles=(analysis=="train" | analysis=="test"),by.gene = F,percentil = F)
...
results <- prettyways(exp, fpathigraphs,verbose=T)
