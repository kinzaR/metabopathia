...
data_pre <- function(exp_file, met_file=NULL, design_file, group1, group2, output_folder, design_type="categorical", analysis="compare", spe="hsa", verbose=FALSE){
  ...
  if(ncol(exp) == 0 || ncol(metabo_data) == 0){
    stop("ERROR: No intersection between sample names in the Expression matrix or metabolomics data and the Design matrix; please check your input data ")
  }
  ...
  if(anyNA(exp) || anyNA(metabo_data)){
  ...
  if (max(trans_exp) == min(trans_exp) || max(metabo_data) == min(metabo_data)) {
